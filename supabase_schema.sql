...
-- Product Variants
create policy "variant_select" on product_variants for select to authenticated using (auth.uid() = user_id);
create policy "variant_insert" on product_variants for insert to authenticated with check (auth.uid() = user_id);
create policy "variant_update" on product_variants for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "variant_delete" on product_variants for delete to authenticated using (auth.uid() = user_id);
...
-- Product Improvements (SKU, Discount, Variants)
alter table products add column if not exists sku text;
alter table products add column if not exists discount_price double precision;
